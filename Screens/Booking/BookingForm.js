...
export default class BookingForm extends React.Component {
  ...
  navigateData = () => {
    this.props.navigation.navigate("PublicProfile", {
      ...
      photographer: this.state.userid,
    });
  };
  ...
  componentWillMount() {
    if (this.props.route.params.id && this.props.route.params.splzId) {
      this.setState({
        userid: this.props.route.params.id,
        specialization: this.props.route.params.splzId,
        splzId: this.props.route.params.splzId,
  ...
  getPhotographerAllData = () => {
    let self = this;
    const photographerTypeUrl = `${serverUrl}showPhotographers.php?singleId=${this.state.userid}`;
  ...
  orderNow = () => {
    ...
    fetch(inserUrl, {
      ...
      .then((responseJson) => {
        ...
        let redirectUrl = `http://192.168.0.116/payment.php?appId=${responseJson}`;
                ...
                <View style={[styles.topPhotographersSelected]}>
                  <View style={styles.imageBoxForm}>
                    <Image
                      ...
                      source={{
                        uri: `http://192.168.0.116/shuttertaps/img/author/${this.state.photographerAllData.profile_pic}`,
                      }}
                    />
